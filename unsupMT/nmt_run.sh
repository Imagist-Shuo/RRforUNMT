...
TRAIN_PARAMS="renew_lr=True,use_pretrained_embedding=False,shared_source_target_embedding=False,device_list=$DEVISES,train_steps=200000,batch_size=4096,save_checkpoint_steps=4000"
...
TRAIN_PARAMS="renew_lr=True,use_pretrained_embedding=False,shared_source_target_embedding=False,device_list=$DEVISES,train_steps=8000,batch_size=4096,save_checkpoint_steps=4000"
TEST_PARAMS="use_pretrained_embedding=False,device_list=$DEVISES,decode_batch_size=64,beam_size=8,decode_alpha=0.6"
...
EVAL_PARAMS="device_list=$DEVICES,eval_batch_size=64"

echo "Start Scoring........."
python3 $RUN_T2T/score.py --input $VALID_FILE $REFS_FILE  \
    --output $EVAL_FILE \
    --vocab $VOCAB_SRC $VOCAB_TRG \
    --model $MODEL_1 \
...
echo "Calculate BLEU........."
#need moses-decoder(http://www.statmt.org/moses/) and sacrebleu (https://github.com/awslabs/sockeye/tree/master/contrib/sacrebleu)
sed 's/@@ //g' $TRANS_FILE | perl $MOSES_PATH/scripts/recaser/detruecase.perl | perl $MOSES_PATH/scripts/tokenizer/detokenizer.perl -l $LANG_TRG | sacrebleu -t $TASK -l $LANG_DIRECTION
